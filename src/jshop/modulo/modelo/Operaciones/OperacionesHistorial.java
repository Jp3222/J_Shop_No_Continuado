...
import jshop.modulo.modelo.Objetos.OHistorial;
 ...
 * @author jp
 */
public class OperacionesHistorial extends Operaciones implements Funciones {

    private final String TABLA = "historial_sesiones";
    private final String[] CAMPOS = Const.getBD_HISTORIAL();
    ...
    @Override
    public ArrayList<OHistorial> getLista(String where) {
        try {
            ResultSet rs = super.get(TABLA, "*", where);
            ArrayList<OHistorial> lista = new ArrayList<>();
            int i;
            while (rs.next()) {
                i = 0;
    ...
    @Override
    public OHistorial getObjeto(String where) {
        try {
            ResultSet rs = super.get(TABLA, "*", where);
